...
export const AdminVehicleSchema = z.object({
  ...
  luggage:          z.coerce.number().int().min(1, 'Mínimo 1 maleta').max(8, 'Máximo 8 maletas').default(2),
  daily_rate:       z.coerce.number().positive('La tarifa diaria debe ser mayor a 0').max(9999),
  sale_price:       z.coerce.number().min(0).optional().nullable(),
  mileage:          z.coerce.number().int().min(0, 'El kilometraje no puede ser negativo').default(0),
  color:            z.string().min(2, 'Color requerido').max(50),
  fuel_consumption: z.string().max(50).optional().default('16.0 km/l'),
