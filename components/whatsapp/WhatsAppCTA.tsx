...
export function WhatsAppCTA({
  ...
  return (
    <motion.a
      ...
      aria-label={`Consultar por WhatsApp sobre el ${brand} ${model} ${year}`}
    ...
    >
      ...
      <span>{isDisabled ? 'WhatsApp no configurado' : label}</span>

      {!isDisabled && (
        <ExternalLink className="w-4 h-4 shrink-0 opacity-70" aria-hidden="true" />
      )}
    </motion.a>
  )
}
